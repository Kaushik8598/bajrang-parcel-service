...
import { usePendingDeliveryReports, useOnlyBranchList, useModulePermissions } from "@/lib/hooks";
...
export default function PendingDeliveryReportPage() {
  const permissions = useModulePermissions("delivery");
  ...
  // Live data fetching hook via GET /report/atDestination
  const { data: apiResponse, isLoading, isFetching } = usePendingDeliveryReports({
    page,
    limit,
    search,
  ...
  const totals = useMemo(() => {
    ...
    bookingRecords.forEach((row) => {
      ...
      totalConfirmedQty += row.trackingStatus?.arrived_at_destination ?? 0;
  ...
  const columns: ColumnDef<ParcelBookingReportItem>[] = [
    ...
    },
    {
      ...
      sortable: true,
      width: "w-20",
      sortValue: (row) => row.trackingStatus?.arrived_at_destination ?? row.parcel ?? 0,
      exportValue: (row) => `${row.trackingStatus?.arrived_at_destination ?? 0}/${row.trackingStatus?.total ?? row.parcel ?? 0}`,
      render: (_, row) => {
        const arrived_at_destination = row.trackingStatus?.arrived_at_destination ?? 0;
        const total = row.trackingStatus?.total ?? row.parcel ?? 0;
        return (
          <span className="text-xs font-semibold text-slate-900">
            {arrived_at_destination}/{total}
          </span>
        );
      },
    },
  ...
  return (
    <div className="space-y-3">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between border-b border-slate-200 pb-2">
            <div className="flex items-center gap-2 text-xs font-bold text-slate-800">
              ...
              <span>Filter Pending Delivery Reports</span>
              {!isAdminOrSuperAdmin && (
      ...
      <DataTable<ParcelBookingReportItem>
        title="Pending Delivery Reports"
        columns={columns}
        data={bookingRecords}
