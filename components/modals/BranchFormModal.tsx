...
const RENT_DUE_OPTIONS = [
  ...
  { value: "5th of Month", label: "5th of Month" },
  { value: "10th of Month", label: "10th of Month" },
  { value: "15th of Month", label: "15th of Month" },
  { value: "20th of Month", label: "20th of Month" },
  { value: "25th of Month", label: "25th of Month" },
...
function getInitialState(mode: "add" | "edit", editData?: BranchUser | null) {
  const bInfo = editData?.branchInfo || {};

  return {
    // Basic user info
    name: editData?.name || "",
    email: editData?.email || "",
    mobile: editData?.mobile || bInfo.mobile1 || "",
    password: "",
    confirmPassword: "",
    ...
    salaryAmount: bInfo.salaryAmount !== undefined ? String(bInfo.salaryAmount) : "",
    Bookingcommission: bInfo.Bookingcommission !== undefined ? String(bInfo.Bookingcommission) : "5.00",
    DeliveryCommission: bInfo.DeliveryCommission !== undefined ? String(bInfo.DeliveryCommission) : "3.50",
    deposite: bInfo.deposite !== undefined ? String(bInfo.deposite) : "",
    commissionTarget: bInfo.commissionTarget !== undefined ? String(bInfo.commissionTarget) : "",

    // Branch Details
    branchName: bInfo.branchName || "",
    branchCode: bInfo.branchCode || "",
    mobile1: bInfo.mobile1 || "",
    ...
    city: bInfo.city || "",
    state: bInfo.state || "GUJARAT",
    pincode: bInfo.pincode || "",
    branchMapLink: bInfo.branchMapLink || "",
    allowPublicBooking: bInfo.allowPublicBooking !== undefined ? String(bInfo.allowPublicBooking) : "true",

    // Rent Details
    monthlyRent: bInfo.monthlyRent !== undefined ? String(bInfo.monthlyRent) : "0",
    rentDueDate: bInfo.rentDueDate || "5th of Month",
    ...
    // Attendance Location
    latitude: bInfo.attendanceLocation?.latitude !== undefined ? String(bInfo.attendanceLocation.latitude) : "",
    longitude: bInfo.attendanceLocation?.longitude !== undefined ? String(bInfo.attendanceLocation.longitude) : "",
    attendanceDistance: bInfo.attendanceLocation?.distance ? String(bInfo.attendanceLocation.distance) : "100 Meters",

    // Documents (Metadata ready for API)
    rentAgreementNumber: bInfo.documents?.rentAgreement?.number || "",
    rentAgreementName: bInfo.documents?.rentAgreement?.fileName || "",
    aadharNumber: bInfo.documents?.aadharCard?.number || "",
    aadharFileName: bInfo.documents?.aadharCard?.fileName || "",
    panNumber: bInfo.documents?.panCard?.number || "",
    panFileName: bInfo.documents?.panCard?.fileName || "",
    bankAccountNumber: bInfo.documents?.bankDetails?.accountNumber || "",
    bankIfscCode: bInfo.documents?.bankDetails?.ifscCode || "",
    bankName: bInfo.documents?.bankDetails?.bankName || "",
    bankAccountHolder: bInfo.documents?.bankDetails?.accountHolder || "",
    passbookFileName: bInfo.documents?.bankDetails?.fileName || "",
    passportPhotoName: bInfo.documents?.passportPhoto?.fileName || "",
    profilePhotoName: bInfo.documents?.profilePhoto?.fileName || "",
  };
}
...
export default function BranchFormModal({
  ...
  const [form, setForm] = useState(() => getInitialState(mode, editData));
  const [errors, setErrors] = useState<BranchFormErrors>({});
  const [showPassword, setShowPassword] = useState(false);
  const [showConfirm, setShowConfirm] = useState(false);
  const [isLocating, setIsLocating] = useState(false);

  // Sync state on open / editData change
  useEffect(() => {
    if (open) {
      setForm(getInitialState(mode, editData));
      setErrors({});
  ...
  const handleSubmit = (e?: React.FormEvent) => {
    ...
    const payload: BranchPayload = {
      ...
      branchInfo: {
        ...
        documents: {
          rentAgreement: { number: form.rentAgreementNumber, fileName: form.rentAgreementName },
          aadharCard: { number: form.aadharNumber, fileName: form.aadharFileName },
          panCard: { number: form.panNumber, fileName: form.panFileName },
          bankDetails: {
          ...
          passportPhoto: { fileName: form.passportPhotoName },
          profilePhoto: { fileName: form.profilePhotoName },
        },
      },
    };
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
            ...
            <div className="p-2 bg-slate-50/50 rounded border border-slate-200 space-y-1.5">
              ...
              <div className="flex items-center gap-1.5">
                <label className="flex-1 cursor-pointer flex items-center justify-center gap-1.5 h-7 px-2 rounded border border-dashed border-slate-300 hover:border-black bg-white text-[11px] font-medium text-slate-700 transition-colors">
                  <Upload className="w-3 h-3 text-slate-500" />
                  <span className="truncate">{form.rentAgreementName || "Upload PDF/Image"}</span>
                  <input
                    type="file"
                    accept=".pdf,image/*"
                    className="hidden"
                    onChange={(e) => handleFileChange("rentAgreementName", e)}
                  />
                </label>
                {form.rentAgreementName && (
                  <button
                    type="button"
                    onClick={() => update("rentAgreementName", "")}
            ...
            <div className="p-2 bg-slate-50/50 rounded border border-slate-200 space-y-1.5">
              ...
              <div className="flex items-center gap-1.5">
                <label className="flex-1 cursor-pointer flex items-center justify-center gap-1.5 h-7 px-2 rounded border border-dashed border-slate-300 hover:border-black bg-white text-[11px] font-medium text-slate-700 transition-colors">
                  <Upload className="w-3 h-3 text-slate-500" />
                  <span className="truncate">{form.aadharFileName || "Upload File"}</span>
                  <input
                    type="file"
                    accept=".pdf,image/*"
                    className="hidden"
                    onChange={(e) => handleFileChange("aadharFileName", e)}
                  />
                </label>
                {form.aadharFileName && (
                  <button
                    type="button"
                    onClick={() => update("aadharFileName", "")}
                    className="p-1 text-slate-400 hover:text-red-500"
                  >
                    <X className="w-3.5 h-3.5" />
                  </button>
            ...
            <div className="p-2 bg-slate-50/50 rounded border border-slate-200 space-y-1.5">
              ...
              <div className="flex items-center gap-1.5">
                <label className="flex-1 cursor-pointer flex items-center justify-center gap-1.5 h-7 px-2 rounded border border-dashed border-slate-300 hover:border-black bg-white text-[11px] font-medium text-slate-700 transition-colors">
                  <Upload className="w-3 h-3 text-slate-500" />
                  <span className="truncate">{form.panFileName || "Upload File"}</span>
                  <input
                    type="file"
                    accept=".pdf,image/*"
                    className="hidden"
                    onChange={(e) => handleFileChange("panFileName", e)}
                  />
                </label>
                {form.panFileName && (
                  <button
                    type="button"
                    onClick={() => update("panFileName", "")}
                    className="p-1 text-slate-400 hover:text-red-500"
                  >
                    <X className="w-3.5 h-3.5" />
                  </button>
            ...
            <div className="p-2 bg-slate-50/50 rounded border border-slate-200 space-y-1.5">
              ...
              <div className="grid grid-cols-2 gap-1.5">
                <label className="cursor-pointer flex items-center justify-center gap-1 h-7 px-1.5 rounded border border-dashed border-slate-300 hover:border-black bg-white text-[10px] font-medium text-slate-700">
                  <Camera className="w-3 h-3 text-slate-500" />
                  <span className="truncate">{form.passportPhotoName || "Passport Photo"}</span>
                  <input
                    type="file"
                    accept="image/*"
                    className="hidden"
                    onChange={(e) => handleFileChange("passportPhotoName", e)}
                  />
                </label>
                <label className="cursor-pointer flex items-center justify-center gap-1 h-7 px-1.5 rounded border border-dashed border-slate-300 hover:border-black bg-white text-[10px] font-medium text-slate-700">
                  <Camera className="w-3 h-3 text-slate-500" />
                  <span className="truncate">{form.profilePhotoName || "Profile Photo"}</span>
                  <input
                    type="file"
                    accept="image/*"
                    className="hidden"
                    onChange={(e) => handleFileChange("profilePhotoName", e)}
                  />
                </label>
              </div>
            </div>
            ...
            <div className="sm:col-span-2 lg:col-span-2 p-2 bg-slate-50/50 rounded border border-slate-200 space-y-1.5">
              ...
              <div>
                <label className="cursor-pointer flex items-center justify-center gap-1.5 h-7 px-2 rounded border border-dashed border-slate-300 hover:border-black bg-white text-[11px] font-medium text-slate-700">
                  <Upload className="w-3.5 h-3.5 text-slate-500" />
                  <span className="truncate">{form.passbookFileName || "Upload Passbook / Cheque"}</span>
                  <input
                    type="file"
                    accept="image/*,.pdf"
                    className="hidden"
                    onChange={(e) => handleFileChange("passbookFileName", e)}
                  />
                </label>
              </div>
            </div>
