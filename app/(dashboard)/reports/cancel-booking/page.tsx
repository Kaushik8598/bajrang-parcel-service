...
import { useCancelBookingReports, useOnlyBranchList, useModulePermissions } from "@/lib/hooks";
...
export default function CancelBookingReportPage() {
  ...
  // Live data fetching hook via GET /report/cancelled
  const { data: apiResponse, isLoading, isFetching } = useCancelBookingReports({
    page,
    limit,
    search,
  ...
  const totals = useMemo(() => {
    ...
    bookingRecords.forEach((row) => {
      ...
      totalConfirmedQty += row.trackingStatus?.cancelled ?? 0;
  ...
  const columns: ColumnDef<ParcelBookingReportItem>[] = [
    ...
    },
    {
      ...
      sortable: true,
      width: "w-20",
      sortValue: (row) => row.trackingStatus?.cancelled ?? row.parcel ?? 0,
      exportValue: (row) => `${row.trackingStatus?.cancelled ?? 0}/${row.trackingStatus?.total ?? row.parcel ?? 0}`,
      render: (_, row) => {
        const cancelled = row.trackingStatus?.cancelled ?? 0;
    ...
    },
    {
      ...
      sortable: true,
      render: (_, row) => {
        const r = row.cancelReason || (row as Record<string, unknown>).cancel_reason;
    ...
    },
    {
      ...
      sortable: false,
      render: (_, row) => {
        const rem = row.cancelRemark || (row as Record<string, unknown>).cancel_remark;
        if (!rem) return <span className="text-slate-400 text-xs">—</span>;
        return (
          <span className="text-xs text-slate-600 line-clamp-1" title={String(rem)}>
            {String(rem)}
          </span>
        );
      },
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between border-b border-slate-200 pb-2">
            <div className="flex items-center gap-2 text-xs font-bold text-slate-800">
              ...
              <span>Filter Cancel Booking Reports</span>
              {!isAdminOrSuperAdmin && (
